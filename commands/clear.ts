module.exports = {
    ...
    run : async(client, message, args) => {
        ...
        if(parseInt(args[0]) > 100) return message.channel.send('The max amount of messages that I can delete is 100').then(msg => msg.delete({timeout: 5000}));
        message.channel.bulkDelete(parseInt(args[0]) + 1).catch(err => console.log(err))
